...
ld=17
# Output path
lpath="/home/operador/grads/ww3_418/ww3testes"
...
fname=lpath+'/spec_teste.nc'
# final path for saving figures
fpath=lpath+'/POINTOUTPUT/'
# --------------------------------------------------------------------------
		...
		for t in range(0,nt):
			...
			for p in range(0,npo):
				...
				if pspdecp==1:
					...
					plt.figtext(0.63,0.88,"Profundidade: "+repr(depth[p])[0:6]+" m",fontsize=9)
					plt.figtext(0.63,0.85,"Velocidade do Vento: "+repr(wnds[t,p])[0:4]+" m/s",fontsize=9)
					plt.figtext(0.63,0.82,"Direção do Vento: "+repr(wndd[t,p])[0:4]+" graus",fontsize=9)
					plt.figtext(0.63,0.79,"Altura significativa de onda: "+repr(4.01*np.sqrt(sum(pwst[t,p,:])))[0:4]+" m",fontsize=9)
					plt.savefig(fpath+'pspec_p'+str(p+1).zfill(3)+'_t'+str(t).zfill(3)+'.jpg', dpi=None, facecolor='w', edgecolor='w', orientation='portrait', papertype=None, format='jpg', transparent=False, bbox_inches=None, pad_inches=0.1)
					plt.close()
				# -------------------------------------------------------------------
				...
				if dspdecp==1:
					...
					if dspec[t,p,:,:].max()<39:
						levels=[2.5,5,7.5,10,12.5,15,17.5,20,22.5,25,27.5,30,32.5,35,37.5]
						plt.contourf(theta, r, ndspec[0:indf,:].T,levels,cmap=palette,norm=colors.Normalize(vmin=0.01,vmax=40,clip=False), extent=[-3,3,-3,3])
#					elif dspec[t,p,:,:].max()<6.0:
...
#						plt.contourf(theta, r, ndspec[0:indf,:].T,levels.round(1),cmap=palette,norm=colors.Normalize(vmin=0.1,vmax=dspec[t,p,:,:].max(),clip=False), extent=[-3,3,-3,3])
					...
					plt.figtext(0.15,0.93,'WAVEWATCH III - Espectro Direcional de Onda    '+str(sdate[t,0]).zfill(4)+'/'+str(sdate[t,1]).zfill(2)+'/'+str(sdate[t,2]).zfill(2)+' '+str(sdate[t,3]).zfill(2)+'Z', fontsize=13)
					ax = plt.gca()
					pos = ax.get_position()
					l, b, w, h = pos.bounds
					...
					cbar.ax.set_ylabel('Densidade de Energia m².s', rotation=270)
					plt.savefig(fpath+'dspec_p'+str(p+1).zfill(3)+'_t'+str(t).zfill(3)+'.jpg', dpi=None, facecolor='w', edgecolor='w',orientation='portrait', papertype=None, format='jpg',transparent=False, bbox_inches=None, pad_inches=0.1)
					plt.close()
					# ===================================================
				...
				if cspdecp==1:
					...
					plt.figtext(0.2,0.93,'WAVEWATCH III - Wave Spectrum  '+str(sdate[t,0]).zfill(4)+'/'+str(sdate[t,1]).zfill(2)+'/'+str(sdate[t,2]).zfill(2)+' '+str(sdate[t,3]).zfill(2)+'Z', fontsize=14)
					ax = plt.gca()
					pos = ax.get_position()
					l, b, w, h = pos.bounds
					cax = plt.axes([l+w+0.04, b+0.01, 0.02, h-0.01]) # setup colorbar axes.
					cbar=plt.colorbar(cax=cax, orientation='vertical',format='%3.1f');cbar.ax.tick_params(labelsize=8)
					plt.colorbar(cax=cax) # draw colorbar
					plt.axes(ax)  # make the original axes current again
					plt.figtext(0.57,0.02,"Wave Direction: Coming from",fontsize=8)
					...
					plt.figtext(0.05,0.84,"Latitude: "+str(round(abs(lat[p]),2))+fllat+"    Longitude: "+str(round(abs(lon[p]),2))+fllon,fontsize=12)
					plt.figtext(0.05,0.75,"Water Depth: "+str(round(depth[p],1))+" m",fontsize=13)
					plt.figtext(0.05,0.68,"Significant Wave Height: "+str(round(4.01*np.sqrt(sum(pwst[t,p,:])),2))+" m",fontsize=13)
					plt.figtext(0.05,0.61,"Wind Speed: "+str(round(wnds[t,p],2))+" m/s",fontsize=13)
					plt.figtext(0.05,0.54,"Wind Direc: "+str(round(wndd[t,p]))+" degrees",fontsize=13)
					plt.savefig(fpath+'dspec_full_p'+str(p+1).zfill(3)+'_t'+str(t).zfill(3)+'.jpg', dpi=None, facecolor='w', edgecolor='w',
