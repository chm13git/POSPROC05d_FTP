...
#                python ftp_adcirc.py hs 69 bg
#
# Outras datas:  python ftp_adcirc.py variável prognóstico_final área ano mês dia 
#                python ftp_adcirc.py hs 69 bg 2020 05 15 
#
# variáveis: hs, tp, tmm, curr, zeta
# prognóstico_final: até 69 (saídas de 30 em 30 min; não pode ser 70 porque o último horário disponível é 23:30 e não 00:00)
# áreas: bg, sig, ssib
# =====================================================================>
...
if len(sys.argv)<3:
	...
	print('|                                               |')
	print('| Ex1: python ftp_adcirc.py hs 69 bg            |')
	print('| Ex2: python ftp_adcirc.py hs 69 bg 2020 08 20 |')
	print('-------------------------------------------------')
	exit()
...
fprog     = int(sys.argv[2])*2 # prognostico final. multipliquei porque o prognostico final real é 138 (para que os prognosticos fiquem iguais                                  aos horarios nas figuras) --> antes, o prog 006 era 03Z, o prog 012 era 06Z e assim por diante. agora, o prog                                  006 é 06Z e assim por diante
...
### Prognóstico final
if int(sys.argv[2]) > 69:
	print('----------------------------------------')
	print('|  O prognóstico final só vai até 69.  |')
...
if sys.argv[1] == "hs":
	...
	while not os.path.exists(nc_file):
	...
	for prog in range(0,fprog+1,6): # range de 6 em 6 = range de 3 em 3 horas
		print(' Processando figura da data:',datai+',', 'prog:', '{0:02d}Z{1}'.format(hh, date_list[dd].strftime('%Y%m%d')))
		...
		# Escala
		cbar = plt.colorbar(cntrf, format='%.2f', fraction=fraction, pad=0.04)
		...
		if prog == 0: # prog 00
			...
			prog = int(prog/2) # essa divisao foi feita para o nome da figura ficar igual ao horario de previsao
			progname = 'Ref: 00Z{0}{1}{2} ({3}) +PROG{4:02d}/Val:{5:02d}Z{6}{7}{8} ({9}) '.format(datai[6:8], dict_title_mon[datai[4:6]], datai[0:4], dict_title_day[dayname], prog, hh, dt[6:8], dict_title_mon[dt[4:6]], dt[0:4], dict_title_day[daynameprog]) 
		plt.title('MODELO ADCIRC/SiWAN - '+'{0} '.format(dict_title_pt[sys.argv[1]])+' - ADCIRC/SWAN Model - '+ \
...
## CORRENTE SUPERFICIAL ================================================
	...
	nc_file   = filepath+datai+'/'+'ADCIRC_{0}_uv_'.format(dict_area[sys.argv[3]])+datai+'_graderegular.nc'
	while not os.path.exists(nc_file):
		print(u'Arquivo não existe... sleep 60s')
		time.sleep(60)
	print('Arquivo existe')
