...
if [ $# -ne 4 ]
then
     echo "Entre com a area modelada (met, ne, pry, ant e inmet), com o horario de referencia (00 ou 12) e com o horario inicial e o prognostico final (24, 48, 71, ..)!!!!"
     exit
fi
AREA=$1
HH=$2
...
echo "Apagando as figuras no diretorio local"
	...
	if [ $HSTART == "00" ];then
		...
		if [ $AREA == "inmet" ];then
			...
			rm -f ${gifdir}/*.gif
			rm -f ${gifdir}/*.png
			rm -f /home/operador/grads/gif/cosmo_inmetzoom_${HH}/*
...
do
	...
	/home/operador/grads/cosmo/scripts/geractl_prog_inmet.sh $AREA $HH $prog
	...
	${dpns5dir}/../scripts/roda_inmet.sh ${AREA} ${HH} ${prog}
...
if [ ${AREA} == "inmet" ];then
	...
	/home/operador/grads/cosmo/scripts/rodacvmt_inmet.sh ${AREA} ${HH}
